...
def coba():
	...
	df.drop(df[df['hospital_name'] ==
	        "Dr. Amino Gondohutomo Regional Psychiatric Hospital"].index, inplace=True)
	...
	df['review_len'] = df['review'].apply(
		lambda x: len(str(x)) - str(x).count(" "))
